        ...
        <div class="cm-article-video-container w-full h-full {{ $container_classes }} relative js--open-video-modal" data-link="{{ $link ?? '' }}" data-title="{{ $model->title_short }}" data-src="{{ $model->file->full_path }}">
            <div class="w-full h-full {{ $classes }} {{ $no_img_bg }}">
                @if($model->thumb)
                    <picture>
                        <!-- WebP версия -->
                        <source srcset="{{ $model->getWebpThumbUrl() }}" type="image/webp">
                        <!-- Оригинал как fallback -->
                        <img class="w-full h-full {{ $classes }} object-cover"
                             src="{{ $model->thumb->full_path }}"
    ...
    @else
        <div class="w-full h-full {{ $classes }}">
            @if($model->file->full_preview_path)
                <picture>
                    <!-- WebP версия -->
                    <source srcset="{{ $model->getWebpImageUrl() }}" type="image/webp">
                    <!-- Оригинал как fallback -->
                    <img class="w-full h-full {{ $classes }} object-cover"
                         src="{{ $model->file->full_preview_path }}"
                         alt="Новости Ингушетии: {{$model->title}}">
                </picture>
            @else
                <picture>

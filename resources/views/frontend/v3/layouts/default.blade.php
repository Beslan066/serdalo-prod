...
<html lang="ru">
    ...
    @if(request()->input('page'))
        Новости Ингушетии - Газета "Сердало" - Страница {{ request()->input('page') }}
    @else
        Новости Ингушетии - Газета "Сердало"
    @endif
    @elseif (Route::is('materials-index'))
        @if(request()->input('page'))
	Материалы и статьи - Газета "Сердало" - Страница {{ request()->input('page') }}
	@else
...
@elseif (Route::is('post-single') && isset($post))
	...
	<meta name="robots" content="all"/>
        ...
        <meta property="og:image" content="{{ isset($post) ? ($post->file->full_path ?? '') : (isset($material) ? ($material->file->full_path ?? '') : '') }}" />
        <meta property="og:url" content="{{ url()->current() }}"/>
        <meta property="og:site_name" content="Сердало"/>
        <meta property="article:author" content="{{
            isset($post, $post->author) 
                ? $post->author->name 
                : (isset($material, $material->author) 
                    ? $material->author->name 
                    : '')
        }}"/>        
        <meta property="article:published_time" content="{{ isset($post) ? $post->published_at : (isset($material) ? $material->published_at : '') }}"/>
        <meta property="article:modified_time" content="{{ isset($post) ? $post->updated_at : (isset($material) ? $material->updated_at : '') }}"/>
        ...
        <meta name="description" content="
            ...
            @elseif (Route::is('posts-index'))
            @if(request()->input('page'))
                Самые актуальные новости Ингушетии на странице. {{ request()->input('page') }}.
            @else
                Самые актуальные новости Ингушетии. Обзор событий дня в Газете 'Сердало'.
            @endif
            @elseif (Route::is('materials-index'))
                @if(request()->input('page'))
                Обзор аналитических материалов и статей в Газете 'Сердало'. Узнайте больше об актуальных проблемах региона {{ request()->input('page') }}.
                @else
    ...
    <body>
        <div class="page_wrapper flex flex-col">
            ...
            <div class="header_wrapper desktop_cik_mobile bg-1">
                ...
                <header class="flex flex-row justify-between items-start md:items-center gap-5 max-w-7xl mx-auto p-5">
                    <img src="{{asset('frontend/img/baneer_cik_mobile.png')}}" alt="">
                </header>
                ...
                <header class="flex flex-row justify-between items-start md:items-center gap-5 max-w-7xl mx-auto p-5">
                    <div class="header-left flex flex-wrap flex-1 items-center gap-2.5 md:gap-5">
                        <div class="header-logo flex items-center">
                            <a href="{{route('home')}}">
                                <img src="{{ asset('frontend/v3/assets/media/base-v2/logo.svg') }}" alt="Сердало логотип">
                            </a>
                        </div>
                        ...
                        <div class="header-right-item flex order-1 sm:order-3 items-center gap-2.5">
                            ...
                            <div class="search-form">
                                <form action="{{ route('search-index') }}" method="$_GET">
                                    ...
                                    <button>Найти<img src="{{asset('images/strelka.svg')}}" alt="Найти"></button>
                                </form>
                            </div>
                            ...
                            <a href="#nav-burger" class="js--open-nav">
                                <img src="{{ asset('frontend/v3/assets/media/base-v2/burger-icon.svg') }}" alt="Меню">
                            </a>
                        </div>
                    </div>
                </header>
                                ...
                                <div class="max-w-7xl mx-auto px-5 flex flex-col md:flex-row gap-5 justify-between">
                                    <div class="order-2 md:order-1 flex flex-col gap-5">
                                        ...
                                        <div>
                                            ...
                                            <a href="serdalo@yandex.ru" class="color-7">{{ __('E-mail') }}: serdalo@yandex.ru</a>
                                        </div>
                                    </div>
                                        ...
                                        <div class="footer-logo flex flex-col justify-end h-full gap-2.5">
                                            <div class="flex md:justify-end">
                                                <div class="flex justify-center items-center w-7 h-7 rounded-full bg-5 ">
                                                    <img class="p-1 w-full h-full" src="{{ asset('frontend/v3/assets/media/base-v2/12+.svg') }}" alt="12+">
                                                </div>
                                            </div>
                                            <div>
                                                <a href="{{route('home')}}">
                                                    <img alt="Логотип Сердало" src="{{ asset('frontend/v3/assets/media/base-v2/footer-logo.svg') }}">
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                    ...
                    <div class="footer-bottom_wrapper bg-1">
                        <div class="footer-bottom max-w-7xl mx-auto p-5 color-9">
                            <div class="w-2/3 flex flex-col gap-5">
                                ...
                                <p>
                                    Реестровая запись СМИ: ЭЛ № ФС 77-78323 от 15.05.2020 г. Учредитель: Государственное автономное учреждение «Издательский дом «Сердало»
                                </p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="menu-input" style="width: 100%">
                            <form method="get" action="{{ route('search-index') }}" class="basis-10/12 flex w-full h-10 justify-between border-2 border-color-3">
                                ...
                                <button class="cm-button cm-button-7 flex gap-1 items-center bg-1 color-2" type="submit">
                                    {{ __('Поиск') }} <img class="h-3/4" src="{{ asset('frontend/v3/assets/media/base-v2/strelka.svg') }}" alt="Поиск">
                                </button>
                            </form>
                        </div>
                    ...
                    <div class="xl:pl-16">
                        ...
                        <div class="">
                            <nav class="modal-footer-nav mb-5 color-4">
                                <ul class="">
                                    <li class="mb-1.5 mobile-hide-page"><a href="{{ route('posts-index') }}">{{ __('Новости') }}</a></li>
                                    <li class="mb-1.5 mobile-hide-page"><a href="{{ route('materials-index') }}">{{ __('Статьи') }}</a></li>

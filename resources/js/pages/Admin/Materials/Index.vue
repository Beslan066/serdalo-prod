        ...
        <div class="px-8 py-8">
            <div class="flex justify-between pb-4">
                ...
                <div class="flex justify-end mb-4">
                    <button-link-primary v-if="is('author')" to="/admin/materials/create">Добавить материал</button-link-primary>
                </div>
            </div>
            <div v-if="isFilterOpen" class="mb-4 bg-white rounded-md shadow p-3">
                <form @submit.prevent="setData(1)">
                    <div class="grid grid-cols-4 gap-2">
                        ...
                        <select-input-2 v-model="filter.translation" :options="{'has': 'Есть', 'no': 'Нет'}" label="Перевод" />
    ...
    <span class="text-sm text-gray-400" v-else>
        Не указан
                        ...
                        <td class="border-t pl-3 pr-6 py-3 flex items-center gap-2">
                            ...
                            <button-link-primary v-if="is('translation_editor') && (!material.translation || !material.translation.id)" :to="'/admin/materials/' + material.id + '/materials_translations/create'"><i class="fas fa-plus-circle"></i> перевод</button-link-primary>
                            <button-action-delete :uri="'/api/admin/materials/' + material.id" @deleted="deleted(material.id)" />
                        </td>
                    </tr>
                    <tr v-if="empty_data">
